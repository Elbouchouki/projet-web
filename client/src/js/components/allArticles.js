...
export default class AllArticles {
  ...
  async reRenderPagination() {
    var htmlPagination = /*HTML*/ ` 
      <div>
        Showing <span class="font-semibold text-gray-900 dark:text-white">${Math.min(
          AllArticles.articlesCount * 10,
  ...
  async reRender() {
    let htmlArticles = "";
    console.log(AllArticles.articlesCount);
  ...
  async render() {
    ...
    <div class="grid mt-10 grid-cols-1 gap-4 place-items-center md:grid-cols-2 lg:grid-cols-3">
        ...
        <div class="mb-5 col-span-full flex flex-col items-center">
          <span id="all-articles-pagination" class="text-sm text-gray-700 dark:text-gray-400">
            ${await this.reRenderPagination()}
          </span>
          <div class="inline-flex mt-2 xs:mt-0">
              <button id="all-articles-pagination-button" class="py-2 px-4 text-sm font-medium text-white bg-gray-800 rounded hover:bg-gray-900 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                  Show More
              </button>
          </div>
        </div>
